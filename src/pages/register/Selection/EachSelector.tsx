		...
		<>
			{info.map((item: {name: string; color?:string})=>(
				<div 
					...
					key={item.name} className={`${clickName===item.name ? "bg-[#fe494e]":(localType!=="color" || regInfo.type.length===0 || (iPhoneColor.length!==0 && iPhoneColor.some((e:any)=>e.color === item.name)) ? "bg-white" : "bg-[#f3f3f3]")} ${localType!=="color" || (iPhoneColor.length!==0 && iPhoneColor.some((e:any)=>e.color === item.name))? "cursor-pointer" : "cursor-default"} ${localType!=="color" || (iPhoneColor.length!==0 && iPhoneColor.some((e:any)=>e.color === item.name))? "pointer-events-auto" : "pointer-events-none"} ${(regInfo.type==="iPhone 13" || regInfo.type.length===0) && item.name==="1TB" ? "hidden" : "flex"} mr-[10px] mt-[10px] p-[10px] w-[max-content] h-[29px] border-solid border-[1px] border-[#d4d9dd] rounded-md flex items-center justify-center`}>
